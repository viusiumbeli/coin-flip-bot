...
class SimulationService(
     ...
     */
    fun initialize(request: SimulationInitRequest): SimulationStateDto = lock.write {
        log.info { "Initializing simulation for ${request.symbol} ${request.timeframe}" }

        val tf = Timeframe.fromLabel(request.timeframe)
            ?: throw IllegalArgumentException("Invalid timeframe: ${request.timeframe}")

        // Load candles from database
        val loadedCandles = candleRepository.findBySymbolAndTimeframeOrderByOpenTimeAsc(
            request.symbol,
            tf
        )
        ...
        val filteredCandles = loadedCandles.filter { candle ->
     ...
     */
    fun advanceCandle(): SimulationStateDto = lock.write {
        checkInitialized()
     ...
     */
    fun previousCandle(): SimulationStateDto = lock.write {
        checkInitialized()
     ...
     */
    fun reset(): SimulationStateDto = lock.write {
        checkInitialized()
     ...
     */
    fun getCurrentState(): SimulationStateDto = lock.read {
        getCurrentStateInternal()
    }
    ...
    private fun getCurrentStateInternal(): SimulationStateDto {
        val currentCandle = if (currentCandleIndex >= 0 && currentCandleIndex < candles.size) {
        ...
        return SimulationStateDto(
            ...
            openPositions = openPositions.map { position ->
                ...
                val unrealizedPnLPercent = if (positionValue > BigDecimal.ZERO) {
